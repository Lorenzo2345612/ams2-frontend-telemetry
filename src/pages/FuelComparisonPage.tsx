...
import { compareLapFuel, getRaceStatus, listRaceIds, FuelComparisonResponse, RaceStatus } from '@/lib/api';
import { FuelComparisonCharts } from '@/components/FuelAnalysisCharts';
...
export function FuelComparisonPage() {
  ...
  const [comparisonData, setComparisonData] = useState<FuelComparisonResponse | null>(null);
  ...
  const handleCompare = async () => {
    ...
    try {
      const data = await compareLapFuel(selectedRace, lap1, lap2);
      setComparisonData(data);
    } catch (err: any) {
      setError(err.response?.data?.detail || 'Failed to compare fuel');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">Fuel Comparison</h1>
